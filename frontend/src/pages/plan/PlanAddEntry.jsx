...
export default function PlanAddEntry({onSave,onCancel,budgetId,categories,setCategories,setIsAdding}) {
    const {
        register,
        handleSubmit,
        reset,
    ...
    return (
        <tr className="bg-zinc-800 relative z-50">
            ...
            <td className="p-2 w-[20%]">
                <select
                    {...register("is_recurring", {required: "Recurring option is required"})}
                    className={`p-1 w-full rounded bg-bglight border ${
                ...
                {errors.is_recurring && (
                    <span className="text-red-500 text-xs">{errors.is_recurring.message}</span>
                )}
            </td>
