...
import createPlan from "../../pages/plans/Actions.jsx";


export default function ProtectedSidebar({setIsOpen, setPlan}) {
    const navigate = useNavigate();
    const {data: plans = [], _, refetch} = usePlans();
    ...
    return (
        <>
            ...
            <div>
                ...
                <div
                    className="blocktext-xl font-bold transition hover:cursor-pointer hover:opacity-70 items-center"
                ...
                <div
                    className={`overflow-hidden transition-all duration-300 ease-in-out my-2 ${
                        openPlan ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"
                    }`}
                >
                    <p className="indent-4 my-2 p-1 hover:cursor-pointer hover:opacity-70" onClick={() => setShowModal(true)}>
                        Add plan
                    </p>
                    {plans.map((plan) => (
                        <div onClick={()=> handleSetPlan(plan)}>
                            <p className={`indent-4 my-2 p-1 hover:cursor-pointer hover:opacity-70`} >
                                {plan.name}
                            </p>
                        </div>))}
                </div>
            ...
            {showModal && (<div className="fixed inset-0 flex items-center justify-center bg-blue-dark/70 z-50">
                <div className="bg-containerbg text-white p-6 rounded-lg max-w-md shadow-lg h-2/4 w-2/4">
